...
void VulkanRenderer::SetModelMatrix(const Matrix4& modelMatrix_, const Matrix4& modelMatrix_2)
{
    pushConst.normalMatrix = MMath::transpose(MMath::inverse(modelMatrix_));
    pushConst.modelMatrix = modelMatrix_;
    pushConst1.modelMatrix = modelMatrix_2;
    pushConst1.normalMatrix = MMath::transpose(MMath::inverse(modelMatrix_2));
    recordCommandBuffer();
}
...
bool VulkanRenderer::OnCreate(){ 
    ...
    Create2DTextureImage("./textures/mario_fire.png", textures[0]);
    Create2DTextureImage("./textures/mario_mime.png", textures[1]);
    

    IndexedVertexBuffer mesh = LoadModelIndexed("./meshes/Mario.obj");
    indexedVertexBuffers.push_back(mesh);

    IndexedVertexBuffer mesh1 = LoadModelIndexed("./meshes/Skull.obj");
    indexedVertexBuffers.push_back(mesh1);
    
    graphicsPipelines.push_back(CreateGraphicsPipeline("./shaders/phong.vert.spv", "./shaders/phong.frag.spv", nullptr));
    graphicsPipelines.push_back(CreateGraphicsPipeline("./shaders/drawNormals.vert.spv", "./shaders/drawNormals.frag.spv", "./shaders/drawNormals.geom.spv"));
    ...
    if (isTexture1 == true) {
        ...
        createDescriptorSets(textures[1]);
    }
...
void VulkanRenderer::OnDestroy() {
    ...
    for (auto item:indexedVertexBuffers) {
        vkDestroyBuffer(device, item.vertBufferID, nullptr);
        vkFreeMemory(device, item.vertBufferMemoryID, nullptr);
        vkDestroyBuffer(device, item.indexBufferID, nullptr);
        vkFreeMemory(device, item.indexBufferMemoryID, nullptr);
    }
...
void VulkanRenderer::cleanupSwapChain() {
    ...
    for (PipeLine p : graphicsPipelines) {
        vkDestroyPipeline(device, p.pipelineID, nullptr);
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        vkDestroyBuffer(device, cameraUboBuffers[i].bufferID, nullptr);
        vkFreeMemory(device, cameraUboBuffers[i].bufferMemoryID, nullptr);
    }
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        vkDestroyBuffer(device, lightUboBuffers[i].bufferID, nullptr);
        vkFreeMemory(device, lightUboBuffers[i].bufferMemoryID, nullptr);
    }
...
void VulkanRenderer::createDescriptorSetLayout() {
    ...
    std::array<VkDescriptorSetLayoutBinding, 3> bindings = { cameraLayoutBinding,LightLayoutBinding, samplerLayoutBinding };
...
PipeLine VulkanRenderer::CreateGraphicsPipeline(const char* vertFile, const char* fragFile, const char* geomFile) {
    PipeLine pipeline{};
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline.pipelineID) != VK_SUCCESS) {
...
void VulkanRenderer::Create2DTextureImage(const char* texureFile, Sampler2Ddata& textureData) {
    ...
    createImage(image->w, image->h, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, 
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureData.Textureimage, textureData.textureimageMemory);
...
void VulkanRenderer::createTextureImageView(Sampler2Ddata& textureData) {
    textureData.textureimageView = createImageView(textureData.Textureimage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
}


void VulkanRenderer::createTextureSampler(Sampler2Ddata& textureData) {
...
IndexedVertexBuffer VulkanRenderer::LoadModelIndexed(const char* filename) {
    ...
    IndexedVertexBuffer tempindexedVertexBuffer;
    createVertexBuffer(tempindexedVertexBuffer,vertices);
    createIndexBuffer(tempindexedVertexBuffer,indices);
    return tempindexedVertexBuffer;
}
...
void VulkanRenderer::createDescriptorSets(Sampler2Ddata& textureData) {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        lightsbufferInfo.buffer = lightUboBuffers[i].bufferID;
...
void VulkanRenderer::recordCommandBuffer() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        vkCmdBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines[0].pipelineID);

        //mario
        VkDeviceSize offsets[] = { 0 };
        VkBuffer vertexBuffers[] = { indexedVertexBuffers[0].vertBufferID };
        vkCmdBindIndexBuffer(commandBuffers[i], indexedVertexBuffers[0].indexBufferID, 0, VK_INDEX_TYPE_UINT32);
        vkCmdBindVertexBuffers(commandBuffers[i], 0, 1, vertexBuffers, offsets);
        vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines[0].pipelineLayout, 0, 1, &descriptorSets[i], 0, nullptr);
        vkCmdPushConstants(commandBuffers[i], graphicsPipelines[0].pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(ModelMatrixPushConst), &pushConst);
        vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(indexedVertexBuffers[0].indexBufferLength), 1, 0, 0, 0);


        //Skull
        VkBuffer vertexBuffers1[] = {indexedVertexBuffers[1].vertBufferID};
        vkCmdBindIndexBuffer(commandBuffers[i], indexedVertexBuffers[1].indexBufferID, 0, VK_INDEX_TYPE_UINT32);
        vkCmdBindVertexBuffers(commandBuffers[i], 0, 1, vertexBuffers1, offsets);
        vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines[0].pipelineLayout, 0, 1, &descriptorSets[i], 0, nullptr);
        vkCmdPushConstants(commandBuffers[i], graphicsPipelines[0].pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(ModelMatrixPushConst), &pushConst1);
        vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(indexedVertexBuffers[1].indexBufferLength), 1, 0, 0, 0);

        vkCmdBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines[1].pipelineID);
        vkCmdPushConstants(commandBuffers[i], graphicsPipelines[1].pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(ModelMatrixPushConst), &pushConst1);
        vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines[1].pipelineLayout, 0, 1, &descriptorSets[i], 0, nullptr);
        vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(indexedVertexBuffers[1].indexBufferLength), 1, 0, 0, 0);
...
void VulkanRenderer::SetLightUBO(const Vec4 pos[MAX_LIGHTS], const Vec4 diffuse[MAX_LIGHTS], uint32_t numberoflights) {
    for (int i = 0; i < numberoflights; i++) {
        lightUBOdata.position[i] = pos[i];
        lightUBOdata.diffuse[i] = diffuse[i];
        lightUBOdata.numberoflights = numberoflights;
    }
}
